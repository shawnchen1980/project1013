...
#    img=np.array(img)
#    ret, binary = cv2.threshold(img,50,255,cv2.THRESH_BINARY)
    binary=img.reshape(1,224,224,1)
    pred_array = model.predict(binary)
    mscore=np.max(pred_array[0])
    result=reverse_map[np.argmax(pred_array[0])] if mscore>0.5 else "unknown"
...
#              int(cap_region_x_begin * frame.shape[1]):frame.shape[1]]  # clip the ROI
        ...
        # Draw the text
#        if(time.time()-lastTime>1 and False):
...
#            lastTime=time.time()
        ...
        target = cv2.resize(thresh1, (224, 224))
#        target = target.reshape(1, 224, 224, 3)
        prediction, score = predictBinaryImage(target)

        cv2.putText(thresh, f"Prediction: {prediction} ({score})", (30, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.5,
                    (255, 255, 255))
...
#                drawing = np.zeros(img.shape, np.uint8)
            ...
            cv2.putText(drawing, f"Show me: {gesture_names[question]} ", (30, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.5,
                    (255, 255, 255))
            if(prediction==gesture_names[question]):
                cv2.putText(drawing, f"You are right ", (30, 80), cv2.FONT_HERSHEY_SIMPLEX, 0.5,
                        (255, 255, 255))
...
#        target = np.stack((thresh1,) * 3, axis=-1)
        target = cv2.resize(thresh1, (224, 224))
#        target = target.reshape(1, 224, 224, 3)
        prediction, score = predictBinaryImage(target)
...
#        print("{} written".format(img_name3))
        filecount+=1
        cv2.imwrite(f"{path}/{filecount}.jpg",thresh1)
        filecount+=1
        thresh2=cv2.flip(thresh1,1)
        cv2.imwrite(f"{path}/{filecount}.jpg",thresh2)
        print(f"img {filecount} done")
#        cv2.imwrite("frames/drawing.jpg", drawing)
#        cv2.imwrite("frames/thresh.jpg", thresh1)
#        cv2.imwrite("frames/img.jpg", img)
#        print(predictImage("frames/thresh.jpg"))
#        print("abc.jpg is out")
#        img_counter += 1
    elif k == ord('q'):
